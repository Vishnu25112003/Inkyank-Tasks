class QuizManager {
  ...
  getPlayerStats() {
    return this.quizState.players.map(player => ({
      id: player.id,
      name: player.name,
      score: player.score,
      ...
      accuracy: player.totalAnswered > 0 ? 
        (player.correctAnswers / player.totalAnswered * 100).toFixed(1) : 0,
  ...
  getCurrentQuestionStats() {
    if (!this.quizState.currentQuestion) return null

    return {
      question: this.quizState.currentQuestion.question,
      totalPlayers: this.quizState.players.filter(p => p.isConnected).length,
      responsesReceived: this.quizState.responses.length,
      timeRemaining: Math.max(0, 
        this.quizState.currentQuestion.timeLimit * 1000 - 
        (new Date() - this.quizState.currentQuestion.sentAt)
      )
    }
  }
